...
async function codexLinuxAppshotNiriWindow(){if(!process.env.NIRI_SOCKET)return null;let e=codexLinuxAppshotRequire(\`node:child_process\`);try{let t=await codexLinuxAppshotExecFile(e,\`niri\`,[\`msg\`,\`--json\`,\`focused-window\`],{encoding:\`utf8\`,timeout:5000,maxBuffer:1048576}),n=JSON.parse(String(t.stdout||\`\`)),r=Array.isArray(n?.layout?.window_size)?n.layout.window_size:[],i=Number(r[0]),a=Number(r[1]),o={x:0,y:0,width:Number.isFinite(i)&&i>0?i:1,height:Number.isFinite(a)&&a>0?a:1};return{window_id:n.id,title:codexLinuxAppshotFirstString(n.title),app_id:codexLinuxAppshotFirstString(n.app_id),pid:n.pid,focused:n.is_focused!==!1,bounds:o,codexLinuxAppshotSource:\`niri\`}}catch(e){codexLinuxAppshotWarn(\`niri-focused-window-failed\`,{message:e instanceof Error?e.message:String(e),stderr:typeof e?.codexStderr===\`string\`?e.codexStderr.slice(0,200):\`\`});return null}}
async function codexLinuxAppshotFocusedWindow(){let e=null;try{e=await codexLinuxAppshotBackendJson([\`windows\`],5000)}catch(e){codexLinuxAppshotWarn(\`backend-windows-failed\`,{message:e instanceof Error?e.message:String(e)})}let t=codexLinuxAppshotFocusedWindowFromReport(e),n=await codexLinuxAppshotNiriWindow();if(n!=null)return{focusedWindow:n,windows:[n],backend:\`niri\`};return t}
...
if(e?.codexLinuxAppshotSource===\`niri\`){let t=await codexLinuxAppshotScreenshotNiri({childProcess:a,fs:n,nativeImage:o,tmpPath:i.join(r.tmpdir(),\`codex-appshot-niri-\${process.pid}-\${Date.now()}-\${Math.random().toString(16).slice(2)}.png\`),windowId:e.window_id});if(t!=null)return t}
...
async function codexLinuxAppshotScreenshotNiri({childProcess:e,fs:t,nativeImage:n,tmpPath:r,windowId:i}){try{let a=[\`msg\`,\`action\`,\`screenshot-window\`,\`--path\`,r,\`--write-to-disk\`,\`true\`,\`--show-pointer\`,\`false\`];i!=null&&a.push(\`--id\`,String(i));await codexLinuxAppshotExecFile(e,\`niri\`,a,{timeout:10000,maxBuffer:1048576});if(!t.existsSync(r)||t.statSync(r).size<=0)return codexLinuxAppshotWarn(\`niri-screenshot-output-empty\`,{}),null;let e=n.createFromPath(r),i=e.getSize();return i.width<=0||i.height<=0?(codexLinuxAppshotWarn(\`niri-screenshot-native-image-empty\`,{}),null):{dataURL:e.toDataURL(),width:i.width,height:i.height,source:\`niri:screenshot-window\`}}catch(e){return codexLinuxAppshotWarn(\`niri-screenshot-failed\`,{message:e instanceof Error?e.message:String(e),stderr:typeof e?.codexStderr===\`string\`?e.codexStderr.slice(0,200):\`\`}),null}finally{try{t.rmSync(r,{force:true})}catch{}}}
